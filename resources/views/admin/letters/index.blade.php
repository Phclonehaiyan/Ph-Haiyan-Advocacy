@extends('admin.layouts.app', ['pageTitle' => 'Letters Editor'])
    ...
    <section class="admin-panel">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="max-w-3xl">
                ...
                <h2 class="admin-heading">Letters Archive</h2>
                <p class="admin-copy">Manage official correspondence, story-backed public records, PDFs, and supporting files with a simpler editor.</p>
            </div>
            ...
            <div class="flex flex-col gap-3 sm:flex-row">
                ...
                <a href="{{ route('admin.letters.create') }}" class="btn-primary justify-center">
                    ...
                    <tbody>
                        @forelse ($records as $record)
                            <tr>
                                ...
                                <td>{{ $record->topic ?: '—' }}</td>
                                <td>{{ $record->published_at?->format('M d, Y') ?: '—' }}</td>
                                <td>
                                ...
                                <td>
                                    <div class="flex flex-wrap gap-3">
                                        <a href="{{ route('admin.letters.edit', $record) }}" class="inline-flex items-center gap-2 rounded-full border border-pine-200 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-pine-900 transition hover:border-pine-300 hover:bg-pine-50">
                                            <x-icon name="edit" class="h-3.5 w-3.5" />
                                            Edit
                                        </a>
                                        <form action="{{ route('admin.letters.destroy', $record) }}" method="POST" onsubmit="return confirm('Delete this letter record?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-sm text-slate-500">No letters found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
